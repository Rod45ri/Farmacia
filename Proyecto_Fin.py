...
class SistemaLogin:
    def __init__(self, root):
        ...
        tk.Label(frame, text="¡Bienvenido de nuevo!", font=("Arial", 16, "bold"), fg="black", bg="#6b7c92").pack(pady=20)
    ...
    def crear_entry(self, frame, texto, es_password=False):
        tk.Label(frame, text=texto, bg="#6b7c92").pack(pady=5)
...
class SistemaFarmacia:
    def __init__(self, master, usuario_id, usuario_nombre):
        ...
        frame_encabezado = tk.Frame(master, bg="#f9f9f9", relief=tk.GROOVE, bd=1)#coloque un botond e actialaoizacion ene el sisteam en tiempo real para que al momento mde afreafara algo no tener ques errar el isetam
        frame_encabezado.grid(row=0, column=0, sticky="ew")
        frame_encabezado.columnconfigure(1, weight=1)  # ← permite que el centro se expanda

        tk.Button(frame_encabezado, text="🔄 Actualizar Todo", command=self.actualizar_todo)\
...
#STOCK  
    ...
    def mostrar_imagen_stock(self, event):
        ...
        if seleccion:
            ...
            nombre_producto = item["values"][1]  # o [0] si usas ID

            ruta_imagen = os.path.join("imagenes_productos", f"{nombre_producto}.png")

            if os.path.exists(ruta_imagen):
                imagen = Image.open(ruta_imagen).resize((120, 90))
